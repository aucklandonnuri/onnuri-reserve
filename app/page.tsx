      ...
      <div className="max-w-md mx-auto space-y-6">
        ...
        <section className="bg-white p-6 rounded-[2.5rem] shadow-xl border-2 border-slate-100 space-y-4">
          ...
          <select 
            ...
            className="w-full p-5 bg-slate-100 border-none rounded-3xl font-black text-lg outline-none"
          >
        ...
        {/* STEP 2: 예약 현황 확인 (수정된 부분) */}
        {(selectedHall && selectedDate) && (
          <section className="space-y-3">
            <h2 className="text-xs font-black text-slate-500 flex items-center gap-2 px-2 uppercase">
              <Clock size={14} /> Current Schedule
            </h2>

            {loadingBookings ? (
              <div className="text-center p-8 font-black text-slate-300">LOADING...</div>
            ) : existingBookings.length === 0 ? (
              <div className="bg-green-50 p-6 rounded-[2rem] border-2 border-green-100 text-center">
                <p className="text-green-700 font-black text-xs uppercase">예약 가능한 빈 시간입니다</p>
              </div>
            ) : (
            ...
            <div className="space-y-3">
              <div className="relative">
                ...
                <input required type="text" placeholder="예약자 성함" value={formData.userName} onChange={e => setFormData({...formData, userName: e.target.value})}
                  className="w-full p-4 pl-12 bg-blue-700 border-none rounded-2xl font-black text-white outline-none" />
              </div>
              <div className="relative">
              ...
              <div className="grid grid-cols-2 gap-3">
                <input required type="time" value={formData.startTime} onChange={e => setFormData({...formData, startTime: e.target.value})}
                  className="w-full p-4 bg-blue-700 border-none rounded-2xl font-black text-white outline-none" />
